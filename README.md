...
proc		
	...
	Rule_Rule2(RS:FSet(SAtt), RO:FSet(OAtt), RA:FSet(AAtt))= !(attribute(subjectid,Doctor) in RS)-> Obligation(RS,RO,RA,email).Response(RS,RO,RA,Deny);
	
	Rule_Rule3(RS:FSet(SAtt), RO:FSet(OAtt), RA:FSet(AAtt))= (attribute(subjectid,CareGiverA) in RS)-> Response(RS,RO,RA,Permit);
